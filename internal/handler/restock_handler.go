...
func (h *RestockHandler) PatchRestock(c *gin.Context) {
	...
	// Validate action
	if req.Action != "approve" && req.Action != "reject" && req.Action != "deliver" {
		c.JSON(http.StatusBadRequest, gin.H{
			"responseCode":    "400",
			"responseMessage": "Invalid action. Must be 'approve', 'reject', or 'deliver'",
		})
		return
	}
	...
	} else if req.Action == "deliver" {
		if err := h.repo.DeliverRestock(c.Request.Context(), restockID, adminID); err != nil {
			if err.Error() == "there is bad request or there is no suitable condition" {
				...
				slog.Error("Failed to deliver restock", "error", err, "id", restockID)
				c.JSON(http.StatusInternalServerError, gin.H{
					"responseCode":    "500",
